//
//  AcceuilDocteurController.swift
//  miniprojetios
//
...
class AcceuilDocteurController: UIViewController ,  UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        performSegue(withIdentifier: "toFiche", sender: indexPath)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        }else if segue.identifier == "toNewSecretaire" {
            let DVC = segue.destination as! AjoutSecController
    ...
    override func viewDidLoad() {
        ...
        datePicker?.addTarget(self, action: #selector(AcceuilDocteurController.dateChanged(datePicker:)), for: .valueChanged)
        
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(AcceuilDocteurController.viewTapped(gestureRecognizer:)))
        ...
        Alamofire.request("http://41.226.11.252:1180/medichelper/gettallredsmedecin.php?idCabinet="+idCabinet!).responseJSON{ response in
    ...
    @IBAction func ajoutSec(_ sender: Any) {
        performSegue(withIdentifier: "toNewSecretaire", sender: self)
    }
}
